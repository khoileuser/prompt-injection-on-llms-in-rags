...
class Visualizer:
    ...
    def create_asr_bar_chart(
        ...
        # Create bars
        bars = ax.bar(names, asrs, color=colors, edgecolor='white', linewidth=1.5)
        ...
        legend_elements = [
            ...
            Patch(facecolor=self.colors['warning'], label='Medium Risk (20-40%)'),
    ...
    def create_trend_line_chart(
        ...
        # Styling
        ax.set_xlabel('Number of Variations Tested', fontsize=12, fontweight='bold')
    ...
    def create_radar_chart(
        ...
        for idx, (model_key, robustness) in enumerate(radar_data.items()):
            values = [robustness.get(cat, 0.5) * 100 for cat in categories]  # Convert to percentage
        ...
        # Title and legend
        ax.set_title(title + '\n(Higher = More Robust)', fontsize=14, fontweight='bold', pad=20)
    ...
    def create_defense_comparison_chart(
        ...
        defense_model_stats = defaultdict(lambda: defaultdict(lambda: {'total': 0, 'success': 0}))
        ...
        models = sorted(set(model for stats in defense_model_stats.values() for model in stats.keys()))
        ...
        ax.set_title(title, fontsize=14, fontweight='bold', pad=20)
        ax.set_xticks(x)
        ax.set_xticklabels(strategy_labels, rotation=45, ha='right', fontsize=10)
